...
{
	// Loads and check the syntax of the script
	...
	// Load the entities and components from scripts
	lua.script_file("./assets/scripts/Level" + std::to_string(levelNumber) + ".lua");
	Logger::Log("Lua script opened");
	...
	while (true) 
	{
		// Loop all the entries of the assets table
		...
		if (assetType == "texture")
		{
			...
			Logger::Log("New texture asset added to the asset store, id: " + assetId);
		}

		if (assetType == "font")
		{
			assetStore->AddFont(assetId, asset["file"], asset["font_size"]);
			Logger::Log("New font asset added to the asset store, id: " + assetId);
		}
		i++;
	}
	...
	// Fetching values from the lua script to C++ variables
	...
	// OLD C++ IMPLEMENTATION (JUST REFERENCE)
